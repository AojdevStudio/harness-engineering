...
from harness_engineering.runner import AgentRunCanceled, AgentRunHandoff, AgentRunner
    ...
    def _reap_finished_workers(self) -> None:
        ...
        for issue_id, future in list(self.futures.items()):
            ...
            if isinstance(entry, RunningEntry):
                ...
                try:
                    future.result()
                except AgentRunHandoff as exc:
                    _mark_attempt_finished(entry, status=AttemptStatus.HANDOFF, handoff_reason=exc.reason)
                    _append_entry_journal(
                        entry,
                        "session_handoff",
                        message=exc.reason,
                        payload={"handoff_reason": exc.reason},
                    )
                    logger.info("worker handed off issue_id=%s issue_identifier=%s reason=%s", issue_id, entry.issue.identifier, exc.reason)
